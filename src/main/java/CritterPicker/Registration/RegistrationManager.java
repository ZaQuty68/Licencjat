...
public class RegistrationManager {
    ...
    public String register(RegistrationRequest request){
        ...
        if(!token.equals("Username") & !token.equals("Email")){
    ...
    public void sendConfirmationAgain(String token, String email, String username){
        String link = "http://localhost:8080/register/confirm?token=" + token;
        emailSender.send(email, buildEmail(username, link));
    }

    @Transactional
    public String confirmToken(String token){
        ConfirmationToken confirmationToken = ctm.getToken(token).orElseThrow(
        ...
        ctm.setConfirmedAt(token);
        aum.enableAppUser(confirmationToken.getAppUser().getEmail());
        return "confirmed";
    }
    ...
    private String buildEmail(String name, String link) {
        return "<div style=\"font-family:Helvetica,Arial,sans-serif;font-size:16px;margin:0;color:#0b0c0c\">\n" +
                ...
                "        \n" +
                "            <p style=\"Margin:0 0 20px 0;font-size:19px;line-height:25px;color:#0b0c0c\">Hi " + name + ",</p><p style=\"Margin:0 0 20px 0;font-size:19px;line-height:25px;color:#0b0c0c\"> Thank you for registering to Critter Picker. Please click on the below link to activate your account: </p><blockquote style=\"Margin:0 0 20px 0;border-left:10px solid #b1b4b6;padding:15px 0 0.1px 15px;font-size:19px;line-height:25px\"><p style=\"Margin:0 0 20px 0;font-size:19px;line-height:25px;color:#0b0c0c\"> <a href=\"" + link + "\">Activate Now</a> </p></blockquote>\n Link will expire in 15 minutes. <p>See you soon</p>" +
                "        \n" +
                "      </td>\n" +
                "      <td width=\"10\" valign=\"middle\"><br></td>\n" +
                "    </tr>\n" +
